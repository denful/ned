...
let
  ...
  users = [
    { name = "alice";   groups = [ "admin" ];  ssh-keys = [ "ssh-ed25519 AAAlice" ];
      hm = { programs.git.enable = true; }; }
    { name = "bob";     groups = [ "deploy" ]; ssh-keys = [ "ssh-ed25519 AAABob" ];
      hm = { programs.vim.enable = true; }; }
    { name = "charlie"; groups = [ "ops" ];    ssh-keys = [ "ssh-ed25519 AAACharlie" ];
      maid = { file.home."greet".text = "hello charlie"; }; }
  ];
in
{
