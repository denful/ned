...
let
  ...
  granted-groups =
    host:
    let
      by-env = access-policy.by-environment.${host.environment} or [ ];
...
in
{
  ...
  fleet-demo.access-c =
    host-stream: user-stream:
    let
      all-users = user-stream.toList;
    in
    host-stream (
      st.flatMap (
        host: st.filterList (has-group (granted-groups host)) all-users (user: st { inherit host user; })
      )
    );
}
