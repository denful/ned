...
in
{
  ...
  flake.tests.nixos.fleet-demo.test-web-prod-1-nginx = {
    ...
    expected = { nginx = true; haproxy = false; };
  };
  ...
  flake.tests.nixos.fleet-demo.test-alice-key-isolation-lb-prod = {
    expr = {
      ...
      no-bob-key     = !(builtins.elem "ssh-ed25519 AAABob"    lb-prod.users.users.alice.openssh.authorizedKeys.keys);
      no-charlie-key = !(builtins.elem "ssh-ed25519 AAACharlie" lb-prod.users.users.alice.openssh.authorizedKeys.keys);
    };
    expected = { has-alice-key = true; no-bob-key = true; no-charlie-key = true; };
  };
  ...
  flake.tests.nixos.fleet-demo.test-charlie-key-isolation-lb-prod = {
    expr = {
      ...
      no-alice-key    = !(builtins.elem "ssh-ed25519 AAAlice"  lb-prod.users.users.charlie.openssh.authorizedKeys.keys);
      no-bob-key      = !(builtins.elem "ssh-ed25519 AAABob"   lb-prod.users.users.charlie.openssh.authorizedKeys.keys);
    };
    expected = { has-charlie-key = true; no-alice-key = true; no-bob-key = true; };
  };
  ...
  flake.tests.nixos.fleet-demo.test-bob-key-isolation-staging = {
    expr = {
      ...
      no-alice-key = !(builtins.elem "ssh-ed25519 AAAlice" web-staging.users.users.bob.openssh.authorizedKeys.keys);
    };
    expected = { has-bob-key = true; no-alice-key = true; };
  };
  ...
  flake.tests.nixos.fleet-demo.test-hm-no-cross-contamination = {
    ...
    expected = { alice-has-git = true; alice-no-vim = true; };
  };
  ...
  flake.tests.nixos.fleet-demo.test-no-class-modules-on-monitor = {
    ...
    expected = { has-hm = false; has-maid = false; };
  };
  ...
  flake.tests.nixos.fleet-demo.test-monitor-has-no-users = {
    ...
    expected = { no-alice = true; no-charlie = true; };
  };
}
