...
let
  inherit (ned) st run;
  inherit (config.fleet-demo) host-c user-c user-hm-c user-maid-c class-imports-c infra-d registry-d access-c nixos-system-d;
  ...
  pipe-backends = env-hosts: host:
    if host.role == "lb"
    then map (h: { addr = h.addr; port = h.httpPort; })
  ...
  pipe-addrs = env-hosts:
    map (h: { hostname = h.name; addr = h.addr; }) env-hosts;

  # Class forwarder: user → nixos.
  # user entry becomes nixos option users.users.${userName}.
  forward-user = { hostName, userName, module }:
    { inherit hostName;
      module = { users.users.${userName} = module; };
    };

  # Class forwarder: homeManager → nixos.
  forward-hm = { hostName, userName, module }:
    { inherit hostName;
      module = { home-manager.users.${userName} = module; };
    };

  # Class forwarder: maid → nixos.
  # maid entry becomes nixos option users.users.${userName}.maid.
  forward-maid = { hostName, userName, module }:
    { inherit hostName;
      module = { users.users.${userName}.maid = module; };
    };
  ...
  main-c = sources:
    let
      ...
      host-nixos   = host-c (sources.infra (st.withPeers (h: h.environment) (env-hosts: host:
        st { inherit host;
    ...
    in
    {
      ...
      nixos = host-nixos
        (st.concat user-as-nixos)
        (st.concat hm-as-nixos)   (st.concat hm-imports)
        (st.concat maid-as-nixos) (st.concat maid-imports);
    };

  result = run { infra = infra-d; registry = registry-d; } main-c;
in
{
